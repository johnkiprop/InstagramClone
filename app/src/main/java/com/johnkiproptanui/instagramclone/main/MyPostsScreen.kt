...
@Composable
fun MyPostsScreen(navController: NavController, vm:InstagramViewModel){
    ...
    Column {
        Column(modifier = Modifier.weight(1f)) {
            ...
            Column(modifier=Modifier.padding(8.dp)) {
                val usernamDisplay=if(userData?.username ==null) "" else "@${userData?.username}"
                Text(text=userData?.name ?:"", fontWeight = FontWeight.Bold )
                Text(text=usernamDisplay)
...
fun ProfileImage(imageUrl:String?, onClick:()-> Unit){
    Box(modifier = Modifier
        ...
        Card(shape = CircleShape, border = BorderStroke(width = 2.dp, color=Color.White),
        modifier = Modifier
            .size(32.dp)
...
@Composable
fun PostList(isContextLoading: Boolean,
             postLoading: Boolean,
             posts: List<PostData>,
             modifier: Modifier,
